	...
	<font color="red">${msg }</font>
	  <form
		...
		method="POST">
			...
			借机时间:<input type="text" name="BorrowOperatDatetime" value="${param.BorrowOperatDatetime }" /><br><br>
			借机批准人ID:<input type="text" name="BorrowPermitPerson" value="${param.BorrowPermitPerson}" />
			借条编号:<input type="text" name="BorrowCheckNo" value="${param.BorrowCheckNo}" />
			归还日期:<input type="text" name="ReturnDatetime" value="${param.ReturnDatetime}" /><br><br>
			是否已归还:<input type="radio" name="Return" value="yes" <c:if test="${param.Return=='yes'}">checked='checked'</c:if>/>是
			<input type="radio" name="Return"  value="no" <c:if test="${param.Return=='no'}">checked='checked'</c:if>>否
			<input type="radio" name="Return"  value="any" <c:if test="${param.Return=='any'}">checked='checked'</c:if>>无
		<input type="submit" value="查询借机明细" />
	...
	<table border="1" width="100%">
		...
		<c:forEach items="${requestScope.list}" var="borrow">
			<tr>
				...
				<td><fmt:formatDate type="time" value="${borrow.borrowOperatDatetime}" pattern="yyyy-MM-dd" /></td>
				...
				<td><fmt:formatDate type="time" value="${borrow.returnDatetime}" pattern="yyyy-MM-dd" /></td>
